        ...
        <div id="page_content" v-show="!cartBlank">
            ...
            <ul class="cart_list">
                <li class="cart_item" v-for="p in pList" data-type="0">
                    <div class="updiv" @touchstart.capture="touchStart" @touchend.capture="touchEnd" @click="skip">
                        <!--<input type="checkbox" class="check ItemCheck" v-model="p.check"  @click.stop.prevent="checkItem(p)"/>-->
                        <span class="check" :class="[p.check?'checked':false]" style="margin-right: 10px;" v-model="p.check"  @click.stop.prevent="checkItem(p)"></span>
                        <!--<span>{{p.check}}</span>-->
                        <div class="cart_detail">
                            ...
                            <div class="cart_msg">
                                <p class="p_name">
                                    <span class="proname">{{p.ProName}}</span><br><span class="promodel" style="font-size: 0.8em;">({{p.ProModel}})</span>
                                </p>
                                <div>
                                    <div class="p_price">
            ...
            <div class="footer_box" style="bottom:51px;">
                <div id="footer">
                    <div class="cart">
                        <div>
                            <span class="check" :class="[allCheck?'checked':false]" v-model="allCheck" @click="checkAll"></span>
                            <p>全选</p>
                        </div>
                    </div>
                    <div class="cart_r r1" v-show="!isWrite">
                        ...
                        <div class="buy f_item">
                            <a @click="goOrder()" id="btn">去结算(<span class="nums">{{allQuantity}}</span>件)</a>
                        </div>
                    </div>
    ...
    export default {
        ...
        methods:{
            ...
            deleteAll () {
                ...
                this.axios.post('account/shopping-cart/delete?token='+this.token,json).then(msg=>{
                    this.btnText = '编辑商品';
                    this.isWrite = false;
                    this.allCheck = true;
                    for(let i=0;i<this.pList.length;i++){
